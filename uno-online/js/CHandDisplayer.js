function CHandDisplayer (iOffsetX,iOffsetY,iX,iY,iPlayerID,oParentContainer,szPlayerName,iSide){
    ...
    this.organizeHand = function(iParent){
        ...
        console.log("_iOffsetX:"+_iOffsetX)
    ...
    this.getDynamicOffset = function(iStartOffset){
        ...
        /*
        if(_aCards.length>6){
            iNewOffset = linearFunction(_aCards.length, 7, 15, iStartOffset, iStartOffset*0.60);
        }
        */
        var iRefWidth = this._getMaxHandWidth();
        
        if(iStartOffset>0){
            var iMaxWidth = iRefWidth;
            if(_aCards.length*iStartOffset > iMaxWidth){
                iNewOffset = iMaxWidth/_aCards.length;
            }
        }else {
            var iMaxWidth = -iRefWidth;
            if(_aCards.length*iStartOffset < iMaxWidth){
    ...
    this._getMaxHandWidth = function(){
        var iWidth;
        switch (_iSide){
            case BOTTOM:{
                    iWidth = 1250;
